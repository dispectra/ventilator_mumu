...
//  Serial.println("Callibrating...");
  ...
  while(digitalRead(CCWL) == LOW) {
    if (millis() - now > 5000) {break;} // 5 detik maks kalibrasi
...
void loop() {
  ...
  if(p_mode!=0 && !digitalRead(EMGS)){
    ...
    if (stepTidal > 0) {
        ...
        Serial.println("eoi"); Serial.flush();
//        Serial.println(stepstop);
        Exhale(stepstop);
        
        while((micros()-now) < timeBreath){}
        
        Serial.println("eoc"); Serial.flush();
//      }
    } else {
      //throw error message
    }
    ...
    if(digitalRead(EMGS)){
      ...
      if(digitalRead(CCWL) == LOW){
        while(digitalRead(CCWL) == LOW) {
          digitalWrite(ENA, HIGH);
          digitalWrite(DIR, !dirInhale);
          unsigned long nowq = millis();
          if (millis() - nowq > 5000) {break;} // 5 detik maks kalibrasi
          digitalWrite(PUL, HIGH);
          delayMicroseconds(1500);
          digitalWrite(PUL, LOW);
          delayMicroseconds(1500);
