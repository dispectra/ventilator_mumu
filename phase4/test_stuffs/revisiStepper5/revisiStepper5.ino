...
void setup() {
  ...
  delayExhale = 500; //delayInhale; // dalam microseconds
...
//  float lookup_vol[] = {223.75, 289.53, 355.72, 410.89, 475.67, 550.83, 606.44, 653.11, 704.17, 748.33, 771.95, 1000}; //release open
  float lookup_vol[] = {223.75, 289.53, 355.72, 410.89, 485.1, 581.3, 606.44, 653.11, 704.17, 748.33, 771.95, 1000}; //release closed
  float lookup_step[] = {450.00, 500.00, 550.00, 600.00, 650.00, 700.00, 750.00, 800.00, 850.00, 900.00, 950.00, 450.00};
...
void Inhale() {
  ...
  float deltaDelayIn = 0;
  
  if(delayInhale > 400){
    if(delayInhale-deltaDelayIn < 400){
      deltaDelayIn = delayInhale - 400;
    }
  } else {
    deltaDelayIn = 0;
  }
  
  float delayInhale2 = delayInhale-deltaDelayIn;
...
//    Serial.println(delayInhale2);/
    ...
    delayInhale2 += 2*deltaDelayIn / (stepTidal);
//    Serial.println(delayInhale2);
  }
