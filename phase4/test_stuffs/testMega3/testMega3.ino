...
void setup() {
	...
	Serial3.begin(57600); //nano sensor
...
void update2Nano() {
	...
	String message2 = '<' + String(Vti) + ','
									+ String(PEEP) + '>';
  Serial.println(message2);Serial.flush();
	Serial3.print(message2); Serial3.flush();
...
// 	pressure_float2 = 0.2*calcDatasheetPressure(pressure_raw) + 0.8*lastPressure;
	pressure_float = pressureq;   //+ 0.2*lastPressure;
...
//  Serial.println("PING/");
  ...
  if(received==""){updated=true;};
    ...
    statq = bufferq[0].toInt();
		pressureq = bufferq[1].toInt();
		flowq = bufferq[2].toInt();
		// PIP_lim = 35;
